...
var ranpizzaarray=[];
 ...
 // Changes the value for the size of the pizza above the slider
  ...
  function changePizzaSizes(size) {
    var i=0;
    var dx = determineDx(ranpizzaarray[i], size);
    var newwidth = ranpizzaarray[i].offsetWidth + dx + 'px';
    for (var i = 0; i < len ; i++) {    
      ranpizzaarray[i].style.width = newwidth;
    }
   
  }
// resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
var resizePizzas = function(size) { 
   ranpizza= document.getElementsByClassName("randomPizzaContainer");
    len =ranpizza.length;
    for (var i = 0;i < len; i++) {
      ranpizzaarray.push(ranpizza[i]);
    }
...
// var pizzasDivarray=[];
// var len1 = pizzasDiv.length;
// for (var i = 0;i < len1; i++) {
//   pizzasDivarray.push(pizzasDiv[i]);
// }
...
//getElementsByClassName
 ...
 //took the below line out of the loop as it remains the same for 200 items
  ...
  for (var i = 0; i < itemslength ; i++) {
    var phase = Math.sin( phaseCalc + icount);
      
    //using translateX instead of style.left
    items[i].style.transform = 'translateX('+(100*phase)+'px)';
    //items[i].style.transform= 'translateZ(' + (100*phase) + 'px)';
    //items[i].style.webkittransform = 'translate3d('+(100*phase),0,0+'px)';
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  var rootelem =document.getElementById("movingPizzas1");// getElementById is used instead of querySelectorAll
 
   var baseelem = document.createElement('img');
    baseelem.className = 'mover';
    baseelem.src = "images/pizza.png";
    baseelem.style.height = "100px";
    baseelem.style.width = "73.333px";
    var icolcount=0;
    for (var i = 0; i < 200; i++) 
      {
        var elem = baseelem.cloneNode(true);// cloneNode()method creates a copy of a node, and returns the clone.
       // elem.basicLeft = (i % cols) * s;
        elem.style.left = icolcount * s + 'px';
        elem.style.top = (Math.floor(i / cols) * s) + 'px';
        rootelem.appendChild(elem) ;

        if (icolcount === 7)
          icolcount = 0;
        else icolcount = icolcount + 1;
  }
